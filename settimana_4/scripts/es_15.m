...
ftrig = 120e6 ./ PSrange; 
f = ftrig ./N_onda;
%funz = @(x) sawtooth(x*2*pi, 1/2) * 1500 + 2050;
funz = @(x) square(x*2*pi) * 1500 + 2050;
...
guad = ampli1./ampli0;
phi = mod(fasi1 - fasi0 - pi, 2*pi) + pi - 2*pi;
...
saveas(gcf,'tmp/provaes15_1.png');
figure;
...
xlabel("Frequenza [Hz]")
ylabel("Guadagno")
saveas(gcf,'tmp/provaes15_2.png');
