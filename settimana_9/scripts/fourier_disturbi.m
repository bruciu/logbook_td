...
%         corr_media = corr_media + (((((yy(ii) - sinusoide(ii)).^2 ...
%         - (yy_smooth(ii) - sinusoide(ii)).^2 - ...
%             x_i(ii).^2 - 2 * delta_y(ii) .* x_i(ii)))).*...
%             ((sum((yy(jj) - sinusoide(jj)).^2 ...
%         - (yy_smooth(jj) - sinusoide(jj)).^2 - ...
%             x_i(jj).^2 - 2 * delta_y(jj) .* x_i(jj)))))./...
%             ((sqrt(sum((yy(ii) - sinusoide(ii)).^2 ...
%         - (yy_smooth(ii) - sinusoide(ii)).^2 - ...
%             x_i(ii).^2 - 2 * delta_y(ii) .* x_i(ii))^2)).*...
%             (sqrt(sum((yy(jj) - sinusoide(jj)).^2 ...
%         - (yy_smooth(jj) - sinusoide(jj)).^2 - ...
%             x_i(jj).^2 - 2 * delta_y(jj) .* x_i(jj))^2)))./(numel(signal_fft)).^2 
%     end
% end
% G = corr_media;
% -----
